...
async function getStreams(tmdbId, mediaType = 'tv', season = 1, episode = 1) {
  ...
  try {
    ...
    const searchUrl = `${BASE_URL}/buscar/${encodeURIComponent(title)}/1/`;
    const searchRes = await fetch(searchUrl, { headers: HEADERS });
    if (!searchRes.ok) return [];
    const searchHtml = await searchRes.text();

    const animeLinkMatch = searchHtml.match(/<a[^>]*href=["'](https?:\/\/jkanime\.net\/[^"']+\/)["'][^>]*title=["']/i) ||
                           searchHtml.match(/href=["'](https?:\/\/jkanime\.net\/[a-zA-Z0-9_-]+\/)["']/i);

    if (!animeLinkMatch) return [];

    let animeBaseUrl = animeLinkMatch[1];
    if (!animeBaseUrl.endsWith('/')) animeBaseUrl += '/';

    const epUrl = `${animeBaseUrl}${epNum}/`;
    const epRes = await fetch(epUrl, { headers: HEADERS });
    if (!epRes.ok) return [];
    const epHtml = await epRes.text();

    const serverUrls = [];
    // Extract video servers from remote iframe elements
    ...
    // Extract servers inside scripts (desu, okru, etc.)
    const scriptRegex = /https?:\\\/\\\/[a-zA-Z0-9_.\-\/]+/gi;
    let sm;
    while ((sm = scriptRegex.exec(epHtml)) !== null) {
      const cleaned = sm[0].replace(/\\\//g, '/');
      if (cleaned.includes('streamwish') || cleaned.includes('vidmoly') || cleaned.includes('voe')) {
        serverUrls.push(cleaned);
      }
    }

    const tasks = serverUrls.map(async (srvUrl) => {
      try {
